...
export default function CaseStudiesCarousel({ items }: { items: WorkItem[] }) {
  ...
  return (
    <>
      <div
        ref={scrollRef}
        className="flex gap-8 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-none"
      >
        {items.map((item, i) => (
          <Link
            key={item.slug}
            href={`/work/${item.slug}`}
            className={`group relative rounded-2xl overflow-hidden snap-start shrink-0 w-[calc((100%-64px)/3)] aspect-[3/4] border border-neutral-300 shadow-lg hover:shadow-none transition-shadow duration-300 bg-neutral-200 ${i % 2 === 0 ? 'mt-4 mb-6 animate-float' : 'mt-10 animate-float-delayed'}`}
          >
            {item.coverImage && (
              <Image
                src={item.coverImage}
        ...
        <div
          className={`relative rounded-2xl snap-start shrink-0 w-[calc((100%-64px)/3)] aspect-[3/4] border border-dashed border-neutral-300 flex items-center justify-center px-8 text-center ${items.length % 2 === 0 ? 'mt-4 mb-6 animate-float' : 'mt-10 animate-float-delayed'}`}
        >
          <span className="inline-flex items-center gap-1.5 text-[11px] tracking-[0.12em] uppercase text-neutral-400">
